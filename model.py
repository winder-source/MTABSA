...
class Aspect_Bert_GAT(nn.Module):
    ...
    def __init__(self, args, dep_tag_num, pos_tag_num):
        ...
        cat_num = sum([args.use_bert_global, args.use_gat_feature, args.use_cross_attn, args.use_ner_feature])
    ...
    def forward(self, input_ids, input_aspect_ids, word_indexer, aspect_indexer, input_cat_ids, segment_ids, pos_class,
                ...
                ner_input_mask, ner_segment_ids):
        ...
        if self.args.use_cross_attn:
            # 正确的做法
            attn_feature = self.cross_attn(dep_feature, dep_feature, ner_feature)[0].squeeze()

            # attn_feature = self.cross_attn(ner_sequence, ner_sequence, dep_out)[0].squeeze()

            # 左边的bert和右边的bert做attention
            # attn_feature = self.cross_attn(feature, feature, ner_feature)[0].squeeze()
            # attn_feature = self.cross_attn(dep_feature, dep_feature, pool_out)[0].squeeze()
